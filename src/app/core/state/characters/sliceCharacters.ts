...
import { Character, CharactersData, CharacterSearchState} from '../../../shared/interfaces/interfaces';
...
const characterSearchSlice = createSlice({
  ...
  initialState,
  reducers: {
    fetchCharactersRequest: (state, action: PayloadAction<{ searchValue: string; currentPage: number }>) => {
      state.loading = true;
      state.error = null;
    ...
    updatePageWithSearchValue: (state, action: PayloadAction<{ currentPage: number, searchValue: string }>) => {
      state.currentPage = action.payload.currentPage;
